...
foreach ($mealCalories as $mealType => $calorieLimit) {
    // Build the API URL for meal search
    ...
    if (isset($data['results'][0])) {
        ...
        $recipeId = $recipe['id'];  // Get the recipe ID for detailed fetching

        // Fetch detailed recipe info
        $recipeDetailUrl = "https://api.spoonacular.com/recipes/$recipeId/information?apiKey=$apiKey";
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, $recipeDetailUrl);
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
        $recipeDetailsResponse = curl_exec($ch);
        curl_close($ch);

        $recipeDetails = json_decode($recipeDetailsResponse, true);
        ...
        $mealPlan[$mealType] = [
            ...
            'image' => $recipe['image'],
            'recipeId' => $recipeId, // Pass the recipe ID to use on the recipe page
